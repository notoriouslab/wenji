...
downstream consumers without monkey-patching method bodies. Wording is
domain-neutral on purpose: wenji is a generic framework and deployments range
from sermon archives to internal policy sites.
"""

from __future__ import annotations

TOPIC_PROMPT = """你是文獻彙整助手。請根據以下與「<tag>{tag}</tag>」主題相關的來源節錄，撰寫一段繁體中文 Markdown 摘要：

- 第一段：用 2-3 句話總結這些來源對「<tag>{tag}</tag>」的核心內容。
...
CONCEPT_PROMPT = """你是文獻彙整助手。請根據以下圍繞概念「<concept>{concept}</concept>」的多來源觀點，回傳一段繁體中文分析：
