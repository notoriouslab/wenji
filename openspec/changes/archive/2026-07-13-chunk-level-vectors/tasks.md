...
- [x] 4.3 落實（以先導 + 權重變體判決：hit@3 持平 → DISCARD，全量依判準免跑）— D5: G4 實驗設計 — 主實驗與權重 fallback 對照分開判。主實驗：backfill 後跑 v3 Part A（hit@1/3/10 + MRR）+ 80q+r14；判準 = **v3 hit@3 上升 且 80q 不劣化** → keep，否則整路 revert；per-題翻轉清單記入 change 附錄
- [x] 4.4 對照實驗（w=1.5/2.0/3.0 全部 hit@3 持平 → discard）—：BM25 零訊號時 vector 系權重 ×1.5 / ×2.0 疊加量測（v3 + 80q 同跑）；獨立 keep/discard；結果入附錄
- [x] 4.5 G4 判決記錄（proposal 附錄）+ 維護者裁決整路 discard — + 維護者確認 → **commit boundary**（判決為 discard 時：revert 檢索側、保留 schema/backfill 基建與否交維護者裁決）
