...
**Why this emerged at apply time, not propose time**：propose 階段 sequential-thinking 推 lifespan 整合時，沒抓到「test fixture 建的是 partial db」這個事實。Phase 3 整合後 19 個 test_web_aggregate / test_eval_baseline_runner 失敗（fixture 只 build articles_meta + doc_vectors，不寫 chunks_fts、不更新 wenji_meta counter，被 L2.c + L1 + L2.b 多重抓到）。
