...
- [ ] 0.1 從 origin/main 切出新 branch `add-doctor-and-startup-check`
- [ ] 0.2 確認 main HEAD 含 PR #5（`356b158`）archive commit

## Phase 1 — Core: `observability.health`

- [ ] 1.1 新增 `src/wenji/core/errors.py` 加 `class StartupError(WenjiError)` （或在 errors module 既有位置 append）
- [ ] 1.2 新增 `src/wenji/observability/health.py`：
...
- [ ] 1.3 `ruff check src/wenji/observability/health.py` + `ruff format` 通過

## Phase 2 — CLI: `wenji doctor`

- [ ] 2.1 新增 `src/wenji/cli/doctor.py`：
...
- [ ] 2.2 `src/wenji/cli/__init__.py`：import `doctor as _doctor` + `app.command(name="doctor", help="Check db consistency (row count vs counter, sample MATCH).")(_doctor.command)`
- [ ] 2.3 `ruff check + format` 通過

## Phase 3 — Startup gate integration

- [ ] 3.1 `src/wenji/web/app.py`：
...
- [ ] 3.3 `src/wenji/cli/search.py`：thin-client fallback in-process Searcher 之前（line ~78 「from wenji.search import Searcher」附近）加 `_ensure_consistency(db_path)`
- [ ] 3.4 確認**未動**: `cli/ingest.py` / `cli/rebuild.py` / `cli/inspect.py` / `cli/stats.py` / `cli/corpus.py` / `cli/classify.py` / `cli/aggregate.py` / `cli/segment.py` / `cli/set_chunk_strategy.py` / `cli/download.py`

## Phase 4 — Tests

- [ ] 4.1 `tests/wenji/test_observability_health.py`：
...
- [ ] 4.2 `tests/wenji/test_cli_doctor.py`：
...
- [ ] 4.3 `tests/wenji/test_web_startup.py`：
