"""Tests for loader_logos_v2 (logos benchmark v2 snapshot loader)."""
...
def test_snapshot_loads_80_candidates():
    cands, meta = load_logos_v2_snapshot(SNAPSHOT_PATH)
    assert len(cands) == 80
    assert meta.logos_source_commit


def test_snapshot_metadata_fields_present():
    _, meta = load_logos_v2_snapshot(SNAPSHOT_PATH)
    assert len(meta.logos_source_commit) >= 8  # git short hash minimum
    assert meta.snapshot_taken_at
    assert meta.snapshot_source_path != ""


def test_each_question_has_at_least_one_path():
    cands, _ = load_logos_v2_snapshot(SNAPSHOT_PATH)
    for c in cands:
...
def test_missing_commit_raises(tmp_path):
    ...
    with pytest.raises(IngestError, match="logos_source_commit"):
        load_logos_v2_snapshot(bad)


def test_missing_file_raises(tmp_path):
    with pytest.raises(IngestError, match="snapshot file not found"):
        load_logos_v2_snapshot(tmp_path / "no.json")
...
def test_invalid_json_raises(tmp_path):
    ...
    with pytest.raises(IngestError, match="not valid JSON"):
        load_logos_v2_snapshot(p)


def test_empty_categories_raises(tmp_path):
    p = tmp_path / "x.json"
    p.write_text(
        json.dumps({"logos_source_commit": "abc", "categories": []}),
        encoding="utf-8",
    )
